...
const JSONBIN_BIN_ID = "68e5a3d743b1c97be95e228b";
const JSONBIN_MASTER_KEY = "$2a$10$IvGjmmJFZX2ZQ6eoZ/42vOTL54rzpy83ya/pnesExdMWpKWV6MDGG";

// Kredensial Admin (TIDAK DITAMPILKAN DI UI)
...
async function deleteCurrentTheme() {
    ...
    const confirmed = await showConfirm(
        `Apakah Anda yakin ingin menghapus tema "${theme.title}"? Tema yang dihapus tidak dapat dikembalikan.`,
        "Hapus Tema"
    );
...
async function saveThemeChanges() {
    ...
    try {
        await updateBin(appState.quizData);
        showError('Perubahan berhasil disimpan');
    } catch (error) {
...
async function deleteQuestion(questionId) {
    ...
    const confirmed = await showConfirm(
        "Apakah Anda yakin ingin menghapus soal ini? Soal yang dihapus tidak dapat dikembalikan.",
        "Hapus Soal"
    );
...
async function resetLeaderboard(themeCode) {
    const confirmed = await showConfirm(
        `Apakah Anda yakin ingin mereset leaderboard untuk ${themeCode}? Semua data ranking akan dihapus permanen.`,
        "Reset Leaderboard"
    );
    ...
    try {
        await updateBin(appState.quizData);
        renderLeaderboardsAdmin();
        showError('Leaderboard berhasil direset');
    } catch (error) {
